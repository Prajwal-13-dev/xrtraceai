...
def create_synthetic_session(brv, labels, meta, segments, anomaly_type):
    ...
    for start, end in segments:
        ...
        # INTERMITTENT CORRUPTION: Don't corrupt the whole segment.
        # Pick a random sub-window of 15 to 45 frames (0.5s to 1.5s) to corrupt.
        # This makes the anomaly a sudden burst, much harder for the CNN to trivially detect.
        ...
        if anomaly_type == "HYPER_SPEED":
            # Variable magnitude multiplier (e.g., 1.8x to 3.5x)
            speed_mult = np.random.uniform(1.8, 3.5)
            ...
            # We need the frame just before sub_start to calculate the first velocity
            if sub_start > 0:
            ...
            # Fix boundary discontinuity right after the corrupted window
            if sub_end < len(synth_brv):
                synth_brv[sub_end, 16:22] = (
                    (synth_brv[sub_end, 5:11] - synth_brv[sub_end - 1, 5:11]) * FRAME_RATE_HZ
                )
            
        elif anomaly_type == "AGGRESSIVE_REACH":
            # Variable magnitude scaling
            reach_mult = np.random.uniform(1.2, 1.8)
            synth_brv[sub_start:sub_end, 5:11] *= reach_mult
            synth_brv[sub_start:sub_end, 16:22] *= reach_mult
            
        # ONLY label the corrupted sub-window as anomalous (3)
